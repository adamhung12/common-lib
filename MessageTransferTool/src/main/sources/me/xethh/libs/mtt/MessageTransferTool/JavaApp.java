...
import me.xethh.libs.mtt.MessageTransferTool.base.messagePool.ExecutionActor;
...
public class JavaApp
{
    ...
    public static class EXE extends ExecutionActor{
        public static class ConnectionError extends RuntimeException implements Serializable {
        ...
        public EXE(ActorRef notification) {
            super(notification);
        }

        @Override
        public ReceiveBuilder buildWith(ReceiveBuilder receiveBuilder) {
            return receiveBuilder.match(MDCEntity.class, mdcEntity -> {
...
//        SpringApplication.run(JavaApp.class, args);
        ...
        ActorRef notifier = ActorBuilder.build(system);
        notifier.tell("hi",ActorRef.noSender());
        // system.actorSelection("akka://TestSystem/user/notification").resolveOne(Duration.ofSeconds(10)).whenComplete(((actorRef, throwable) ->  actorRef.tell("hello",ActorRef.noSender())));
